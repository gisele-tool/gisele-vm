  ...
  class VM
    class ProgList
      ...
      private
        ...
        def decode(h)
          h[:pc]       = h[:pc].to_sym
          h[:waitfor]  = h[:waitfor].to_sym
          h[:waitlist] = ::Kernel.eval(h[:waitlist])
          h[:input]    = ::Kernel.eval(h[:input])
          Prog.new(h)
        end
        ...
        def encode(prog)
          h = prog.to_hash
          h[:pc]       = h[:pc].to_s
          h[:waitfor]  = h[:waitfor].to_s
          h[:waitlist] = h[:waitlist].inspect
          h[:input]    = h[:input].inspect
          h
        end
